...
const addCart = (req, res) =>{
    ...
    if(!products) return cart.save([]);
    cart.save(products);
    res.json({message: 'Carrito agregado'});
}

const deleteCart = (req, res) => {
    const id= Numbre(req.params.id);
...
const getProducts = (req, res) => {
    ...
    if(isNaN(id)) return req.status(400).send({message: 'Ingresa el ID de un carrito listado'});
...
const deleteProduct = (req, res) =>{
    const id = Number(req.params.id);
    const id_prod = Number(req.params.id);
